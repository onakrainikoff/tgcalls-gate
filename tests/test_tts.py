...
class TestTtsService:
    ...
    def test_tts_process_use_cache(self, config_tts):
        ...
        test_en_audio_file_path = tts_service.process('test_en', TextToSpeech(text='Hello world! It is test!', lang='en'))
        assert os.path.isfile(test_en_audio_file_path)
        test_ru_audio_file_path = tts_service.process('test_ru', TextToSpeech(text='Привет мир! Это тест!', lang='ru'))
        ...
        test_en_audio_file_path = tts_service.process('test_en_2', TextToSpeech(text='Hello world! It is test!', lang='en'))
        assert os.path.isfile(test_en_audio_file_path)
        test_ru_audio_file_path = tts_service.process('test_ru_2', TextToSpeech(text='Привет мир! Это тест!', lang='ru'))
        assert os.path.isfile(test_ru_audio_file_path)
